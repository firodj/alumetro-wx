...
BEGIN_EVENT_TABLE(wxVideoTerminal, wxWindow)
	...
	EVT_LEFT_DOWN(wxVideoTerminal::OnMouseLeftDown)
	EVT_MOTION(wxVideoTerminal::OnMouseMove)
	EVT_MOUSEWHEEL(wxVideoTerminal::OnMouseWheel)
...
void wxTermLine::SetCharAttr(int pos, char chr, int atr) {
	attrs[pos] = atr;
	chars[pos] = chr;
	if (atr > 0) visible_width = pos +1;
	dirty = true;
};

//////////////////////////////////////////////////////////////////////////////////////////
wxVideoTerminal::wxVideoTerminal(wxWindow *parent, wxWindowID id):
	wxWindow(parent, id, wxDefaultPosition, wxDefaultSize, wxVSCROLL|wxWANTS_CHARS ),
...
void wxVideoTerminal::OnMouseLeftDown(wxMouseEvent &evt)
{
	if (evt.ShiftDown() && m_isSelecting) {
		m_ptEndSelect.x = evt.GetPosition().x/m_font_size.GetWidth() ;
		m_ptEndSelect.y = evt.GetPosition().y/m_font_size.GetHeight() +m_scrollpos_row;
	} else {
		m_ptBeginSelect.x = evt.GetPosition().x/m_font_size.GetWidth();
		m_ptBeginSelect.y = evt.GetPosition().y/m_font_size.GetHeight() +m_scrollpos_row;
		m_ptEndSelect = m_ptBeginSelect;
		m_isSelecting = false;
	}
...
void wxVideoTerminal::OnMouseMove(wxMouseEvent &evt)
{
...
void wxVideoTerminal::OnMouseWheel(wxMouseEvent &evt)
{
	...
	//wxLogDebug("Wheel: %d/%d", evt.GetWheelRotation(), evt.GetWheelDelta());
}
...
void wxVideoTerminal::DoTab()
{
	...
	static const int fixed_tab_space = 8;
...
void wxVideoTerminal::DoPrint(wxChar chr)
{
	...
	if (i < line.width)
		line.SetCharAttr(i, chr, m_cursor_attr.value);
...
void wxVideoTerminal::DoIndex()
{
	...
	if (m_screen_cur.y+1 > m_botmargin_y ) // m_screen_size.GetHeight()
	{
		if (m_topmargin_y == 0)
		{
			...
			y_removed = m_topscreen_row + m_topmargin_y;
	} else
		m_screen_cur.y++;

	if (y_removed >= 0) {
		m_lines.RemoveAt( y_removed );
		m_lines.Insert( wxTermLine(m_screen_size.GetWidth()), m_topscreen_row+m_botmargin_y );
		if (m_isSelecting) {
			// FIXME: Test Me!
			if (m_ptBeginSelect.y >= y_removed) m_ptBeginSelect.y--;
			if (m_ptEndSelect.y >= y_removed) m_ptEndSelect.y--;
		}
	}
